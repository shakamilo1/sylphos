...
4. 根据 `model_version` 选择模型目录：
   - `base` 对应 `pretrained_models/Fun-CosyVoice3-0.5B`。
   - `rl` 对应 `pretrained_models/Fun-CosyVoice3-0.5B-rl`。
   - 不建议把 `pretrained_models/base` 或 `pretrained_models/rl` 软链接直接传给 `AutoModel`。
...
BASE_MODEL_DIR=/home/<user>/sylphos_services/cosyvoice3/pretrained_models/Fun-CosyVoice3-0.5B
RL_MODEL_DIR=/home/<user>/sylphos_services/cosyvoice3/pretrained_models/Fun-CosyVoice3-0.5B-rl
DEFAULT_MODEL_DIR=/home/<user>/sylphos_services/cosyvoice3/pretrained_models/Fun-CosyVoice3-0.5B
