...
当前 CLI fallback 使用本机 OpenClaw 已确认的 agent 入口，不再使用早期占位的 `openclaw <text>`：

```powershell
openclaw agent --message "<text>" --json --timeout <seconds>
```

默认追加 `--json`，便于 Bridge 解析结构化输出；默认追加 `--timeout`，与 `OPENCLAW_TIMEOUT_SECONDS` 保持一致。默认不追加 `--deliver`，因为 Sylphos 自己负责语音回复、UI 显示、普通日志和审计日志分流。默认也不追加 `--local`，因为 `openclaw agent` 默认走 Gateway agent turn；只有在本地配置中显式设置 `OPENCLAW_CLI_LOCAL = True` 时才启用 embedded local agent。
...
CLI 模式仍然是 fallback / 实测入口；长期 HTTP/Gateway 模式继续复用 PR #15 的 `OpenClawHTTPClient`。Bridge 仍通过 `subprocess.run(..., cwd=request.workspace)` 支持 workspace，不在当前任务里猜测 OpenClaw CLI 是否有独立 workspace 参数。
...
- OpenClaw CLI 当前按 `openclaw agent --message ... --json --timeout ...` 构造；后续如 CLI 参数变化再集中调整 `_build_cli_command()`；
