...
- 接收来源无关的文本或任务请求；
- 生成 `OpenClawRequest`；
- 调用 OpenClaw CLI 或未来 Gateway；
- 捕获 stdout、stderr、exit code 和 timeout；
- 将 OpenClaw 输出整理成 `OpenClawResult`；
...
`http` / `gateway` 模式会复用 PR #15 的 `OpenClawHTTPClient`，走 OpenAI-compatible HTTP Gateway。`websocket` / `ws` 模式当前只返回未实现结果，不强行写死未知协议。长期方向是由 typed Gateway WebSocket 承载结构化任务、实时事件、电脑控制结果、确认请求和进度更新。
...
## 7. OpenClawResult 如何分流

`OpenClawResult` 至少包含以下分流字段：
...
TTS 不直接朗读 raw stdout/stderr。成功但输出过长时，只朗读“处理完成，详细结果已记录或显示”。失败、超时、dry-run 和需要确认都有各自的短语音反馈。
...
- `medium`：创建文件、修改项目文件、运行常见命令等；
- `high`：删除/覆盖文件、修改系统配置、未知脚本、联网下载并执行、发送邮件/消息、读取敏感目录、涉及 token/password/secret/ssh key 等。

当前策略：

- high：如果 `context={"confirmed": True}` 不存在，则返回 `needs_confirmation`，不执行 OpenClaw；
...
- 输出解析当前支持纯文本和简单 JSON 对象，复杂事件流需要后续增强；
- 风险分类是基础关键词规则，不是完整安全沙箱；
- 高风险确认只通过 `context["confirmed"]` 预留接口表达，尚未接 UI/语音确认；
